...
function trigger_submit_on_ctrl_enter(e) {
	...
	chrome.extension.sendRequest({url: reply.getAttribute('orig_href')}, function(response) 
	{
		var s = response.html
		try {
		...
		chrome.extension.sendRequest({url: 'http://www.livejournal.com/talkpost_do.bml',
			method: "POST",
			headers: {
